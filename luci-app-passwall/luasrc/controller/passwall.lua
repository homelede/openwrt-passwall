...
function index()
	...
	entry({"admin", "vpn", appname, "auto_switch"}, cbi(appname .. "/client/auto_switch"), _("Auto Switch"), 3).leaf = true
	...
	entry({"admin", "vpn", appname, "node_subscribe"}, cbi(appname .. "/client/node_subscribe"), _("Node Subscribe"), 94).dependent = true
	...
	entry({"admin", "vpn", appname, "link_load_temp"}, call("link_load_temp")).leaf = true
...
function reset_config()
	...
	luci.http.redirect(luci.dispatcher.build_url("admin", "vpn", appname))
end
